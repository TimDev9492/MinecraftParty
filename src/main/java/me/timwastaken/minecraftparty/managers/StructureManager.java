...
public class StructureManager {
    ...
    private static List<File> structureFiles;

    public static void init() throws IOException {
        structureFiles = loadStructureFiles(MinecraftParty.getInstance().getConfig().getString("structure_database_path"));
    }

    private static List<File> loadStructureFiles(String directory) throws IOException {
        List<File> files = new ArrayList<>();
        File structureDirectory = new File(directory);
        for (File structureFile : structureDirectory.listFiles()) {
            if (structureFile.isFile()) {
                files.add(structureFile);
            }
        }
        return files;
    }

    public static List<File> getStructureFiles() {
    ...
    public static void placeStructure(File file, Location target, boolean mirrorX, boolean mirrorZ) throws IOException {
        ...
        for (int stateNum = 0; stateNum < states.length; stateNum++) {
            ...
            if (blockTag.containsKey("Properties")) {
                ...
                Set<Map.Entry<String, Tag<?>>> propertySet = properties.entrySet();
                Iterator<Map.Entry<String, Tag<?>>> propertyIterator = propertySet.iterator();
                for (int p = 0; p < propertySet.size(); p++) {
